...
describe("context policy", () => {
  ...
  it("rejects output and traversal arguments on read-only command tools", async () => {
    const context = { workspaceRoot: "/srv/app", signal: new AbortController().signal, async resolvePath(value: string) { return value; } };
    await expect(gitDiffTool.execute({ action: "diff", args: ["--output=/tmp/leak"] }, context)).rejects.toThrow("Read-only");
    await expect(wordpressPluginTool.execute({ action: "list", args: ["../../outside"] }, context)).rejects.toThrow("Read-only");
  });
});
